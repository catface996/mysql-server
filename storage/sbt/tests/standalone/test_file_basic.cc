...
/** @file test_file_basic.cc
 Basic Test for SBT File Operations Interface

 This test verifies basic file operations using standard C library functions
...
// Test configuration
const char* TEST_FILE_PATH = "/tmp/sbt_file_basic_test.sbt";
...
uint32_t simple_crc32(const unsigned char *data, size_t length) {
...
public:
  Basic_SBT_file() : fd(-1), is_open(false) {}
  ...
  int create(const char *name) {
    ...
    // Calculate checksum
    header.checksum = 0;
    header.checksum = simple_crc32((const unsigned char*)&header, sizeof(header) - sizeof(header.checksum));
...
public:
    ...
    void print_summary() {
        ...
        if (passed_count == test_count) {
            std::cout << "\n🎉 ALL BASIC FILE OPERATIONS TESTS PASSED! 🎉" << std::endl;
        } else {
...
bool test_file_creation_deletion() {
    ...
    // Verify file exists
    struct stat buffer;
    if (stat(TEST_FILE_PATH, &buffer) != 0) {
    ...
    // Test file deletion
    result = Basic_SBT_file::delete_file(TEST_FILE_PATH);
    if (result != SBT_SUCCESS) {
...
bool test_file_metadata() {
    ...
    // Check file size
    off_t file_size = file.get_file_size();
    if (file_size <= 0) {
    ...
    if (file_size < (off_t)SBT_HEADER_SIZE) {
    ...
    if (!Basic_SBT_file::file_exists(TEST_FILE_PATH)) {
        std::cerr << "file_exists should return true for existing file" << std::endl;
        cleanup_test_files();
        return false;
    }
    
    if (Basic_SBT_file::file_exists("/tmp/non_existent_file.sbt")) {
...
// Main test runner
int main() {
    std::cout << "=== SBT File Operations Interface Test (Basic) ===" << std::endl;
